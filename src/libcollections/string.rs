...
// FIXME(conventions): ensure bit/char conventions are followed by String's API
...
impl String {
    ...
    /// Reserves capacity for exactly `additional` more bytes to be inserted in the given
    /// `String`. Does nothing if the capacity is already sufficient.
    ///
    /// # Panics
    ///
    /// Panics if the new capacity overflows `uint`.
