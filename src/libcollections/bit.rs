...
use core::iter::{mod, CloneIteratorExt};
use core::num::Int;
...
fn match_words <'a,'b>(a: &'a Bitv, b: &'b Bitv) -> (MatchWords<'a>, MatchWords<'b>) {
    ...
    if a_len < b_len {
        (a.blocks().enumerate().chain(Repeat::new(0u32).enumerate().take(b_len).skip(a_len)),
         b.blocks().enumerate().chain(Repeat::new(0u32).enumerate().take(0).skip(0)))
    } else {
        (a.blocks().enumerate().chain(Repeat::new(0u32).enumerate().take(0).skip(0)),
         b.blocks().enumerate().chain(Repeat::new(0u32).enumerate().take(a_len).skip(b_len)))
    }
}
