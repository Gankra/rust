...
impl<K: Eq + Hash<S>, V, S, H: Hasher<S>> HashMap<K, V, H> {
    ...
    ///
    /// assert_eq!(map.remove_equiv(&EvenOrOdd { num: 1 }), Some("foo"));
    /// assert_eq!(map.remove_equiv(&EvenOrOdd { num: 2 }), None);
    ///
    /// ```
    #[experimental]
...
mod test_map {
    ...
    fn test_pop_equiv() {
        let mut m = HashMap::new();
        m.insert(1i, 2i);
        assert_eq!(m.remove_equiv(&KindaIntLike(1)), Some(2));
        assert_eq!(m.remove_equiv(&KindaIntLike(1)), None);
    }
    ...
    fn test_insert() {
        ...
        assert_eq!(m.insert(1i, 3i), Some(2));
        assert_eq!(m.insert(1i, 4i), Some(3));
    }
